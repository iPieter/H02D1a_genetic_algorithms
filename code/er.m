...
% TSP tours
%
% Syntax:  NewChrom = er(OldChrom, XOVR)
%
% Input parameters:
...
function NewChrom = er(OldChrom, XOVR);
...
[rows,cols]=size(OldChrom);
   ...
   for row=1:2:maxrows
	...
	if rand<XOVR			% recombine with a given probability
		NewChrom(row,:) =cross_er([OldChrom(row,:);OldChrom(row+1,:)]);
		NewChrom(row+1,:)=cross_er([OldChrom(row+1,:);OldChrom(row,:)]);
	else
